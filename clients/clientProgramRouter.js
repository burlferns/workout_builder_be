...
const {validTokenCheck, validBodyCheck, validRecordIdCoachIdCheck} 
...
router.post('/', 
  validTokenCheck, 
  validBodyCheck(['program_id', 'client_ids']), 
    ...
    Clients.extractClientsInProgram(programId)
      .then(array => {
        ...
        if (alreadyLinked === true) {
          res.status(400)
            .json({ error: `one or more of the clients are already linked to program with id: ${programId}`});
        } else {
  ...
  validRecordIdCoachIdCheck(coach_id, 'programs', [req.body.program_id])
    .then(returnObject => {
      if (returnObject.idExists === false) {
        res.status(400)
          .json({ message: `program with id: ${req.body.program_id} does not exist` });
      } else if (returnObject.validCoachId === false) {
        res.status(400)
          .json({ message: `you do not have access to program with id: ${req.body.program_id}` });
      } else {
    ...
    .then(returnObject => {
      if (returnObject) {
        if (returnObject.idExists === false) {
          ...
          res.status(400)
            .json({ message: `you do not have access to client with id: ${returnObject.badCId}` });
        } else {
          next();
        }
      }
